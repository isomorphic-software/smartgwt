 ...
 * passing an  explicit null value for the binary field. <P> <b>Restricting upload sizes</b>  <p> 
 ...
 * 10MB).  <p style="text-indent: 25px"> DSRequest.maxUploadFileSize: 104857600 <p> <b>Processing
 * File Uploads with server-side business logic</b> <P> Server-side business logic that processes
 * file uploads may retrieve upload files via the server side API
 * dsRequest.getUploadedFile(<i>fieldName</i>).  The uploaded file is returned as an instance of
 * ISCFileItem, which provides access to a Java InputStream as well as metadata about the file
 * (size, name).   See the server-side JavaDoc (com.isomorphic.*) for details. <P> <span
 * style="color:red;font-weight:bold;">NOTE:</span> request processing engines such as Struts may
