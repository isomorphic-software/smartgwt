 ...
 * before the record is added to the grid. <P> Saving a new record and editing it can be done via
 * {@link com.smartgwt.client.data.DataSource#addData DataSource.addData} followed by a call to
 * {@link com.smartgwt.client.widgets.grid.ListGrid#startEditing ListGrid.startEditing} once the
 * record has been saved. <li> edit new records via a separate {@link
 ...
 * com.smartgwt.client.data.DataSource#updateCaches DataSource.updateCaches}. <P> Usually the best
 * approach is to avoid this situation by editing such records in a form or other control until
 * they are valid rather than showing invalid records in a grid.  However, if such records need to
 * be considered invalid, one approach is to take field values and add them as editValues via
 * {@link com.smartgwt.client.widgets.grid.ListGrid#setEditValues ListGrid.setEditValues}.  At
