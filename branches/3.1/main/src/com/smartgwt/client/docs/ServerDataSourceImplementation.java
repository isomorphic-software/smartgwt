 ...
 * and executing it in its own thread</li> <li>If the <code>server.properties</code> attribute
 * <code>datasources.pool.enabled</code> is set, DataSource instances are automatically cached in
 * an in-memory pool</li> <li>If pooling is not switched on, DataSource instances are constructed
 * new as required,  but they are constructed using cached configuration for better performance. 
 ...
 * without restarting the server.</li> <li>The safe way to obtain a DataSource instance is to use 
 * <code>RPCManager.getDataSource</code>.  If pooling is enabled, this is the only way to  obtain
 * a DataSource instance with the guarantee that it will be returned to the pool at  the end of
 * request processing.</li> <li>If you are using the <code>DynamicDSGenerator</code> to provide
 * DataSources to the  framework dynamically:<ul> <li>You are expected to use one of the
 ...
 * <code>DataSource.fromXML()</code>. </li></ul> </li></ul>
 */
