...
public class SectionStack extends VLayout  implements com.smartgwt.client.widgets.layout.events.HasSectionHeaderClickHandlers {
     ...
     * link{isc.AutoTest.getLocator()}, how should sections within this stack be identified? By default if section has a
     * specified ${isc.DocUtils.linkForRef('Section.name')} this will always be used. For sections with no name, the following
     * options are available: <ul> <li><code>"title"</code> use the title as an identifier</li> <li><code>"index"</code> use
     * the index of the section in the sections array as an identifier</li> </ul>  If unset, and the section has no specified
