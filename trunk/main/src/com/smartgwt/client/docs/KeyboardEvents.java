 ...
 * cases where default behavior cancellation is not always possible include: <ul><li>Some function keys (<code>f1, f3,
 * f5,</code> etc) which trigger native browser behavior.         [These can be suppressed in Internet Explorer and Mozilla
 * Firefox but not in some other          browsers such as Safari / Chrome, etc]</li>     <li>Some accelerator key combos
 * such as <code>Alt+f3</code></li>     <li>The "Meta" key (the <code>Windows</code> / <code>Apple</code> key to show OS
 * level menu)     </li> </ul> If you do want to include functionality for these keys in your application, we'd recommend 
 * testing against your expected users' browser types. It is also worth considering whether by changing the functionality
 * of these standard browser keys you may provide an unexpected  user experience (for example a user may press "f5" in an
 * attempt to reload the application and be surprised by this triggering some alternative functionality in your
 * application).
