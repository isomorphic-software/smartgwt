 ...
 * JavaScript.  Hence standard &#010 {@link i18n internationalization} techniques such as using JSTL tags in a JSP
 * remain&#010 applicable.&#010 <br>&#010 The Smart GWT Architecture even allows you to capture all the gradations of
 * cacheability&#010 from completely static (changes once per application rollout) to completely dynamic&#010 (timestamp). 
 * In the example above, the user name wouldn't actually change for the lifetime&#010 of the page, so could be loaded once
 * only.&#010 <br>&nbsp;</dd>&#010&#010 <dt><b>Performance: Minimal Server State</b></dt>&#010 <dd>&#010 Any architecture
 * that relies on component descriptions being generated by the server&#010 must track a great deal of state, which, in the
 * Smart GWT Architecture, is either&#010 completely eliminated or greatly reduced.&#010 <br>&nbsp;</dd>&#010&#010
 * <dt><b>True Presentation / Business Logic separation</b></dt>&#010 <dd>&#010 The RPCResponse object represents the
 * client's exact, minimal needs for server data.&#010 This is much easier to understand and to audit than a slew of .jsp
 * files which access and&#010 update miscellaneous state.  It is also far easier to spot reusable patterns of data
 * access,&#010 which in server-side HTML generation systems often end up as duplicated code.&#010
 ...
 * com.smartgwt.client.docs.ClientOnlyDataSources Client Only DataSources}.&#010 <br>&nbsp;</dd>&#010 </dl>&#010 <br>&#010
 * <h3>Refinements</h3>&#010 <br>&#010 <b>Creating vs Showing a View</b>&#010 <br>&#010 Many views will be shown to the
 * user repeatedly, for example, the user may repeatedly switch&#010 back and forth between two panes of a TabSet.  In that
 * usage it makes sense to make a&#010 distinction between <i>creating</i> a view and <i>showing</i> an existing view. 
 * When&#010 showing an existing view, the same components and/or data may be able to be reused.&#010 <br>&#010 In the
 * following variant on the original example, we only create the Window object and&#010 do the RPC to retrieve the user
 ...
 * <P>&#010 You can use  FileLoader.loadJSFiles to load a set of JavaScript files&#010 compromising an application module
 * that defines a set of related views.  The loaded&#010 JavaScript files may define new component classes and new
 * DataSources in addition to&#010 defining new views and their associated logic.
 */
