 ...
 * SmartGWT makes it easy to implement this ideal handling <i>without</i> having to implement session timeout handling in
 * every codepath that contacts the server, by providing central notification of session timeout, and the ability to
 * re-send a transaction that encountered session timeout. <P> When session timeout occurs, typical authentication systems
 * intercept requests to protected resources and return a page that tells the user that login is required.  With a
 * background RPC this page won't be shown to the user. <P> To enable SmartGWT to detect that session timeout has occurred,
 * a snippet of HTML called the loginRequiredMarker must be added to the session timeout response sent by the
 * authentication system.  The loginRequiredMarker is standardized and does not need to be customized for your deployment. 
 * It can be added to an HTML page without visual effect, or if the response sent on session timeout is not an HTML page,
 * simply embedded in any other type of response, or sent as the entirety of the response.  The loginRequiredMarker is in
 * isomorphic/login/loginRequiredMarker.html. <P> When SmartGWT detects the loginRequired marker, the transaction that
 * encountered session timeout is put on hold, and {@link com.smartgwt.client.rpc.RPCManager#loginRequired} is called.  At
 * this point you have a few options: <ol>  <li> Leave the SmartGWT application and take the user to the login page, by
 ...
 * logged in. This is simple, does not drop context, but is not seamless.  <li> Use a SmartGWT interface, typically a
 ...
 * <B>Authentication via background SmartGWT server RPC/DMI</B> <P> If you are using the SmartGWT Java server and your
 ...
 * liesure, or even log the user back in automatically.
