 ...
 * Response), by using a variety of possible approaches to  connect to the ultimate data provider. <p> SmartGWT supports,
 * out of the box, codeless connectivity to various kinds of common data providers, including SQL and Hibernate.  SmartGWT
 ...
 * to in-browser persistence engines  such as <a href=http://gears.google.com>Google Gears</a>. <p> <b>Choosing a Data
 * Integration Approach</b><p> This section aims to help you decide which of the many possible data integration approaches
 * is best for your particular circumstances.  The recommendations given here will guide you to the approach that involves
 * the least effort.<p> <img src="skin/dataIntegrationFlowchart.png"> <p> <ul> <li>If you have a Java server</li>
 * <ul><li>If your ultimate storage is a SQL database</li>   <ul>       <li>If you are already committed to Hibernate, use
 * the Hibernate DataSource</li>       <li>Otherwise, use the SQL DataSource</li>       <li>Be sure to read the overview of
 * {@link com.smartgwt.client.docs.SqlVsJPA 'SQL DataSource vs JPA'} and           other technologies.  If you ultimately
 * decide not to use the SQL or            Hibernate DataSource, write a {@link
 * com.smartgwt.client.docs.WriteCustomDataSource 'generic DataSource'}</li>       <li>Derive DataSource definitions from
 * existing tables using            ${isc.DocUtils.linkForExampleId('sqlWizard', 'Visual Builder wizards')} or the Batch
 * DataSource            Generator tool.  Or, generate tables from DataSource definitions you create by           
 * hand</li>   </ul>     <li>If your ultimate storage is not SQL, write a          {@link
 ...
 * <ul><li>Firstly, consider approaches for {@link com.smartgwt.client.docs.UsingServerWithNonJava 'using the SmartGWT
 * Server framework with non-Java technologies'}.         Using one of these approaches will allow you to leverage SmartGWT
 * Server features          that you might otherwise have to write and maintain yourself</li>     <li>If you cannot use the
 * SmartGWT Server, or decide you do not wish to</li>   <ul><li>If you are not obliged to use a pre-existing network
 * protocol, use the            {@link com.smartgwt.client.data.RestDataSource}</li>       <li>If you are obliged to use
 * WSDL (Web Service Definition Language) and you have WSDL           experience, use the {@link
 * com.smartgwt.client.data.WSDataSource}.  We do not recommend this approach           except in these circumstances;
 * generally, RestDataSource is a better approach</li>       <li>Otherwise, use a {@link
 * com.smartgwt.client.docs.ClientDataIntegration 'client-side data integration'} approach</li>   </ul> </ul> </ul> <p><br>
 * <b>RPCs: Unstructured Server Communication</b> <P> SmartGWT also supports "unstructured" client-server operations. 
 * These  {@link com.smartgwt.client.rpc.RPCRequest}s (Remote Procedure Call Requests) are a low-level, very flexible 
 * mechanism for custom client-server communications.  In an nutshell, RPCRequests: <ul> <li> may contain arbitrary data
 * <li> are always initiated by custom code (a call to {@link com.smartgwt.client.rpc.RPCManager#send}), and have their
 * responses handled by custom code (the callback passed to <code>send()</code>) </ul> <P> RPCRequests are relatively rare.
 * Most client-server communications are better done in a  structured fashion using a {@link
 * com.smartgwt.client.data.DSRequest} (DataSource Request).  Note that <em>any</em> RPCRequest can alternatively be framed
 * as a {@link com.smartgwt.client..dataSource#fetch};  depending on the circumstances, this may be more convenient. <P>
 * See the {@link com.smartgwt.client.rpc.RPCManager} documentation for further information on RPCRequests.
 */
