 ...
 * MultiFileItem FormItems that enable users to upload one or more files as a background operation, without leaving the
 * current page <li>server-side support that allows binary uploads to be treated as a normal DataSource field values, with
 * all other aspects of server-side integration unchanged <li>built-in SQL DataSource support that can store and retrieve
 * uploaded files from SQL databases </ul> The following documentation assumes you are using the Smart GWT Java Server.  If
 * you are not, skip to the sections near the end of this document. <P> <b>Single file upload: "binary" field type</b> <P>
 * To use Smart GWT's client-server upload system, you use a DataSource field of {@link
 * com.smartgwt.client.data.DataSourceField#getType type} "binary".  By default, a DynamicForm bound to a DataSource with a
 * field of type "binary" will use the {@link com.smartgwt.client.widgets.form.fields.FileItem}, which displays a standard
 * HTML &lt;input type="upload"&gt; form control. <P> When you call {@link
 ...
 * com.smartgwt.client.types.FieldType field type "binary"}. <li> if you have server-side business logic, the inbound
 * request may be routed to your business logic via RPCManager dispatch or DMI declarations as normal, your business logic
 * will receive a normal DSRequest, and you are expected to provide a normal DSResponse. </ul> <P> Client-side callbacks,
 * such as the callback passed to saveData(), fire normally. <P> <b>Processing File Uploads with server-side business
 * logic</b> <P> Server-side business logic that processes file uploads may retrieve upload files via the server side API
