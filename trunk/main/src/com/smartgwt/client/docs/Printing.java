...
/**
 * <h3>Printing</h3>
 * The browser's built-in support for printing will at best print what you see, which in the case of a web application will
 * often be useless, illegible, or partial. <P> Smart GWT has specialized printing support that can take any page built
 * with Smart GWT components and provide a reasonable printed view.  The default printed view: <P> <ul> <li> renders
 * components without clipping or scrolling regions, so that a scrolling grid shows all loaded rows <li> removes certain
 * decorative images, such as image-based backgrounds, which may print poorly in black and white <li> converts editing
 * controls into static representations of the data being edited <li> removes interactive elements such as buttons and
 * menus, which don't work on paper and waste space </ul> The default printed view can be customized with settings and
