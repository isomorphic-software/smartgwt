...
/**
 * ListGrids support automatic validation of edited cells / records. This group is a collection  of APIs related to the validation subsystem. <p> Default validation occurs in response to the user navigating between edit cells  (see {@link com.smartgwt.client.widgets.grid.ListGrid#getValidateByCell validateByCell}) or whenever edited values are to be committed to the server for saving.  Standard validation can also be triggered for a cell or row programatically at any time.<br> When standard validation occurs, {@link com.smartgwt.client.widgets.grid.ListGridField#getValidators validators} will be run on each cell to be validated.<br> In addition to this standard behavior developers can add custom errors to fields via {@link com.smartgwt.client.widgets.grid.ListGrid#setFieldError} / {@link com.smartgwt.client.widgets.grid.ListGrid#setFieldError}.
