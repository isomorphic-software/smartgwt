 ...
 * in production (though don't rule out using the SQL DataSource in  production - see {@link
 * com.smartgwt.client.docs.SqlVsJPA 'this discussion'} of the advantages of doing so}.   As you switch from using Built-in
 * DataSources to Custom Operations, no client-side code  changes will be required, because the client cares only about the
 * DataSource definition,  not the data store which the data is ultimately retrieved from. <br><br> <b>Data Managers:
 * ResultSet and ResultTree</b> <br><br> Data Managers manage datasets retrieved from DataSources.  Data Managers are
 * automatically created by DataBound components, but can be created directly when more control is needed. <br><br> Data
 * Managers provide load-on-demand for datasets too large to be loaded on the client, automatically invoking DataSource
 * operations as necessary to retrieve data as it is requested, and optionally fetching ahead to anticipate further
 * requests.   Data Managers will automatically perform actions locally when it is possible, for example, a sort action can
 * be performed locally with a complete cache.  Data Managers also automatically manage the consistency of the client-side
 * cache, observing update operations performed against DataSources and integrating updated rows automatically. <br><br>
