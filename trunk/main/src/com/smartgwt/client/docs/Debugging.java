 ...
 * diagnostic&#010 messages logged by the SmartGWT framework. The "Logging Preferences" menu lets you&#010 enable and
 * disable SmartGWT's built-in diagnostics in several categories. Because&#010 important diagnostic messages may be logged
 * at any time, you should have the Developer&#010 Console open whenever you are working with SmartGWT (and you should
 ...
 * critical to be familiar with the diagnostic categories built-in to SmartGWT - &#010 you will use them in most debugging
 ...
 * development platform.  In FireFox,&#010 JavaScript errors are reported both in the SmartGWT Developer Console and in the
 * FireFox&#010 JavaScript console, which can be opened by entering the special&#010 url "javascript:" in the address
 * bar.&#010 <P>&#010 Isomorphic currently recommends that every SmartGWT developer install either the &#010 <a
 ...
 * occured.&#010 <P>&#010 In Internet Explorer, when JS errors occur, SmartGWT is able to report full stack traces&#010 in
 * the Developer Console.  This can be invaluable when your code triggers a JS error&#010 in the SmartGWT libraries
 ...
 * Developer Console can be used&#010 to inspect the current state of a SmartGWT application.  Any SmartGWT or browser&#010
 * built-in API can be called from the "Evaluate JS Expression" area, and the results will&#010 be intelligently summarized
 * (via {@link com.smartgwt.client..Log#echo}).  For example, simply typing a&#010 component's ID and pressing the "Eval
 * JS" button will give you a dump of it's current&#010 property values.  &#010 <P>&#010 Many, many SmartGWT APIs can be
 ...
 * com.smartgwt.client.data.ResultSet#get} can be called to inspect the current values on records.  In addition,&#010 new
 * application code can be tried out, for example, you might repeatedly instantiate a new&#010 component, trying variants
 * on the properties you could give it.&#010 <P>&#010 <b>Inspecting transient application state with logs</b>&#010 <P>&#010
 * Transient state, such as the values of local variables in a method that is crashing, can be&#010 sent to the Developer
 * Console via using the {@link com.smartgwt.client..Log} class.  For example, to dump the&#010 value of the local variable
 * "request": &#010 <pre>&#010     isc.logWarn("request is: " + isc.echo(request));&#010 </pre>&#010 <P>&#010 It's a good
 * idea to dump the values of local variables in any method that is crashing or&#010 behaving unexpectedly.&#010 <P>&#010
 * Note the use of {@link com.smartgwt.client.util.isc#logWarn} above: in typical debugging sessions,&#010 it's best&#010
 * to simply use <code>logWarn</code> method to output diagnostics to ensure your message will&#010 not be suppressed by
 * log priority settings.&#010 <P>&#010 NOTE: never use the native <code>alert()</code> method to output diagnostics. 
 * Among other&#010 issues, <code>alert()</code> can affect timing, masking or altering the behavior you were&#010 trying
 * to debug.  SmartGWT's logging system doesn't suffer from these problems and&#010 provides much more control.&#010
 * <P>&#010 <h4>Issue Reports</h4>&#010 <P>&#010 If you believe you've discovered a bug in SmartGWT or you are having
 * trouble using&#010 SmartGWT APIs, you can report it at <a href='http://forums.smartclient.com/'
 ...
 * browser(s), operating system(s) and SmartGWT version(s) you experience the error&#010 on (SmartGWT version is available
 * in the lower-left handle corner of the Developer&#010 Console)&#010 </ul>&#010 Then, include <b>either</b> a standalone
 * test case (see below), <b>or</b>:&#010 <ul>&#010 <li> For JS errors, Stack traces from Firebug (for Firefox) or the
 * Developer Console (for&#010 IE), as covered under "Debugging JavaScript Errors" above&#010 <li> Results of calling
 * <code>echo()</code> on local variables or other application&#010 state you think is relevant (see "Inspecting
 * Application State" above)&#010 <li> What server platform and {@link com.smartgwt.client.docs.ClientServerIntegration
 * 'databinding approach'} you&#010 are using, if applicable&#010 <li> contents of the SmartGWT Developer Console "Log
 ...
 * area of the&#010 Developer Console on some specified page within the unmodified SmartGWT SDK,&#010 demonstrating your
 * issue&#010 <li> an .html or .jsp file that can be dropped at a specified location into an unmodified&#010 SmartGWT SDK
 ...
 * <P>&#010 Submitting a standalone test case removes any ambiguity as to whether there is a bug in&#010 SmartGWT or a bug
 ...
 * relevant SmartGWT&#010 subsystem, often as the new highest priority task.  In addition, the process of preparing a&#010
 * test case very often allows you to solve the issue yourself.&#010 <P>&#010 There are two approaches to test case
 * preparation:&#010 <ol>&#010 <li> Add code to an existing SmartGWT example until you can reproduce the problem&#010 <li>
 ...
 * preconditions that you can report to Isomorphic)&#010 <li> For any issue that isn't cosmetic, revert to a default
 * SmartGWT skin&#010 <li> For any necessary RPC/DataSource interactions, spoof the interaction with one of these&#010
 ...
 * Finally, move your .html file into the stock SmartGWT SDK along with any remaining&#010 dependencies and verify the
 ...
 * could enable&#010 or disable them only when you need the relevant diagnostics, like SmartGWT's built-in&#010 diagnostic
 ...
 * <P>&#010 As with SmartGWT's built-in diagnostics, you may choose to log certain messages in your&#010 custom category at
 * the <code>DEBUG</code> level and a lesser number of messages at the&#010 <code>INFO</code> level, to create different
 ...
 * (<code>echo()</code> and&#010 <code>echoAll()</code>) are available on every SmartGWT component and Class.  Hence,&#010
 ...
 * value "this" is not always set to a SmartGWT component, for example, in some&#010 kinds of callbacks (eg {@link
