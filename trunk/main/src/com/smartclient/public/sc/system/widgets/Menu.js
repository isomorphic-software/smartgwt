...
setFields : function (fields, a,b,c,d) {
    ...
    return this.invokeSuper(isc.Menu, "setFields", fields, a,b,c,d);
},
...
getIcon : function (item) {
	...
	if (item.icon) {
		var icon = (this.itemIsEnabled(item) || !item.disabledIcon ? item.icon : item.disabledIcon);
		return this.imgHTML(icon,
							(item.iconWidth? item.iconWidth : this.iconWidth),
							(item.iconHeight? item.iconHeight : this.iconHeight));
	}
	if (item.checked) {
		return this.getCheckmarkImage(item);
	}
