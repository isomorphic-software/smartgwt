 ...
 * content as possible with caching headers.  Setting these headers allows the end-user's browser and any intermediary
 * proxies to keep a local copy of the file across browser reloads, ensuring the subsequent visits require almost no
 * fetches from the server.  Eliminating these fetches is also important for the reasons explained under File Assembly
 * below.  <p> <u><b>File Assembly</b></u> <p> Modern browsers limit the number of HTTP connections they use to fetch the
 * assets required to render your pages - typically to just 2 connections.  As a result, connection latency can play a much
 * higher role than available bandwidth and the two connection limit effectively drastically limits the user's usable
 * bandwidth.  For this reason, it's important to minimize the number of HTTP requests made to the server.  The File
 * Assembly services make it easy to deliver multiple css and javascript file via one HTTP response.  Assembling files
 ...
 * pop an alert, bug shows a different lock icon to indicate that not all content is served from a secure server.
 * <li>Firefox will not write images fetched via SSL to disk, even if expires headers are set. But it will cache them in
 ...
 * up limiting the download bandwidth by delayng the download of other assets required to render the page. <p> 2.  Whenever
 * possible, don't sent the "Cache-Control" header to "private" - this tells intermediary proxies to not allow caching of
 * the specified content, eliminating that cache as a useful intermediary for other users behind it. <p> 3.  For a good
 * overview of performance considerations, see this post: <a href='http://www.die.net/musings/page_load_time/'
