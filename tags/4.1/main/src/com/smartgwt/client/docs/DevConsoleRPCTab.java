 ...
 * true</code> in your {@link com.smartgwt.client.docs.Server_properties server.properties}       
 * file.  This will cause server timing information to be gathered and returned for          every
 * DSRequest from every client</li>     <li>Enable the built-in RPCs "areServerTimingsTracked" and
 * "trackServerTimings" via the         <code>RPCManager.enabledBuiltinMethods</code> setting of
 * your           {@link com.smartgwt.client.docs.Server_properties server.properties}        
 * file (these builtin RPCs should already be enabled in a development environment).          When
 * these built-in RPCs are enabled, server timing data can be switched on and          off on a
 * per-client basis, via a checkbox in the Developer Console.</li>     </ul>  </li> </ul> With
