 ...
 * of {@link com.smartgwt.client.data.DSRequest#getData data} so that the server can discern which
 * fields the user actually changed. However, the grid always includes the original field values
 * in the dsRequest as {@link com.smartgwt.client.data.DSRequest#getOldValues oldValues}. <P> Note
 * that although it is possible to load DataSource data without actually declaring a {@link
 * com.smartgwt.client.data.DataSourceField#getPrimaryKey primaryKey field}, a primaryKey must be
 * declared for editing and saving.  The values of primaryKey fields is how Smart GWT identifies
 * the changed record to the server. <P> <b>Validation</b> <P> Any time saving is attempted,
 * validation is automatically triggered.  Values entered by the user will be checked against the
 * {@link com.smartgwt.client.widgets.grid.ListGridField#getValidators validators} and the {@link
 * com.smartgwt.client.data.DataSourceField#getValidators validators}. Any invalid values abort an
 * attempted save. <P> Similar to editing and saving, validation can be done on row transitions or
 * on cell transitions by setting {@link
 ...
 * a per-row basis, use {@link com.smartgwt.client.widgets.grid.ListGrid#setEditorCustomizer
 ...
 * com.smartgwt.client.data.DataSourceField#getEditorType editorType} for special coding patterns
 * that apply in this case.  <P> <b>Binary Fields</b> <P> The ListGrid will automatically show
 * "view" and "download" icon buttons for binary field types (see {@link
