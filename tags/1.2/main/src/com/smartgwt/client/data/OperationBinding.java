...
public class OperationBinding extends DataClass {
     ...
     * href='http://jakarta.apache.org/velocity/docs/vtl-reference-guide.html'
     * onclick="window.open('http://jakarta.apache.org/velocity/docs/vtl-reference-guide.html');return false;">VTL
     * Reference</a> and <a href='http://jakarta.apache.org/velocity/docs/user-guide.html'
     * onclick="window.open('http://jakarta.apache.org/velocity/docs/user-guide.html');return false;">Velocity User Guide</a> 
     * for an overview of how to use VTL. <p> The Velocity context is pre-populated with the following variables - you can pass
     * these verbatim as arguments, or call methods on these objects and pass the resulting values: <ul> <li>dsRequest:
     ...
     * <code>DSRequest</code> object and call <code>getStartRow()/getEndRow()</code> in the body of the method.
     *
     ...
     * Overrides the {@link com.smartgwt.client.data.DataSource#getQualifyColumnNames qualifyColumnNames} property. Only
     * applicable to dataSources of {@link com.smartgwt.client.data.DataSource#getServerType 'serverType'} "sql".
     *
    ...
    /**
     * Whether a user must be authenticated in order to access this operation.  For details of  what is meant by
     * "authenticated", see {@link com.smartgwt.client.data.DataSource#getRequiresAuthentication requiresAuthentication}. <P>
     * To protect access to an entire operationType (eg, all "fetch" operations), declare an operationBinding with
     * <code>requiresAuthentication="true"</code>, {@link com.smartgwt.client.data.OperationBinding#getOperationType
     * operationType} set to the operationType to be protected, but no {@link
     * com.smartgwt.client.data.OperationBinding#getOperationId operationId}.  This will then prevent access to the "fetch"
     * operationType unless another {@link com.smartgwt.client.data.OperationBinding} declares requiresAuthentication="false"
     * with a specific {@link com.smartgwt.client.data.OperationBinding#getOperationId 'operationId'}.
     *
    ...
    /**
     * Comma-separated list of user roles that are allowed to invoke the operation described by&#010 this
     * operationBinding.&#010 <P>&#010 Whether the current user has a given role is determined by calling the standard
     * Java&#010 servlets method <code>httpServletRequest.isUserInRole()</code>, hence works with both simple&#010 J2EE
     * security (realms and form-based authentication) and JAAS (Java Authentication &&#010 Authorization Service).&#010
     * <P>&#010 If you wish to use a role-based security scheme that does not make use of the servlet API's &#010 standards,
     * SmartGWT Server also implements the <code>setUserRoles</code> method&#010 on <code>RPCManager</code>.  You can use this
     * API to tell SmartGWT that all the &#010 requests in the queue currently being processed are associated with a user who
     * has the roles&#010 you supply; in this case, SmartGWT will not attempt to resolve the user's roles via &#010
     * <code>httpServletRequest.isUserInRole()</code>.&#010 <P>&#010 If there an operationBinding declared for a given
     * operationType which does not have an&#010 {@link com.smartgwt.client.data.OperationBinding#getOperationId operationId},
     * that is, it is the default operationBinding for the type, then any other&#010 operationBinding of the same type is
     * assumed to have the same setting for&#010 <code>requiresRole</code> as the default operationBinding for the
     * operationType.  For&#010 example, given these declarations:&#010 <pre>&#010     &lt;operationBinding
     * operationType="fetch" requiresRole="manager"&gt;&#010           ... settings ...&#010     
     * &lt;/operationBinding&gt;&#010     &lt;operationBinding operationType="fetch"
     * operationId="fetchWithExtraFields"&gt;&#010           ... settings ...&#010      &lt;/operationBinding&gt;&#010
     * </pre>&#010 The second operationBinding requires the "manager" role even though there is no explicit&#010
     * <code>requiresRole</code> declaration.  To prevent the "manager" role being required by the&#010 second
     * operationBinding, add <code>requireRole=""</code>.&#010 <P>&#010 Note that if {@link
     * com.smartgwt.client.data.DataSource#getRequiresRole requiresRole} is set, all operations on the DataSource require&#010
     * the roles set for the DataSource as a whole, even if they declare individual&#010 <code>requiresRole</code>
     * attributes.&#010 <P>&#010 This property is valid only for a server-side DataSource when using the SmartGWT Server.
     *
