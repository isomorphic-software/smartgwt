 ...
 * com.smartgwt.client.widgets.grid.ListGrid#startEditing} or {@link
 * com.smartgwt.client.widgets.grid.ListGrid#startEditingNew}. <P> <b>New record creation</b> <P> By default, editing is
 * restricted to existing records.  Setting {@link com.smartgwt.client.widgets.grid.ListGrid#getListEndEditAction
 * listEndEditAction} to "next" allows the user to create new records by simply navigating off the end of the dataset with
 * the keyboard.  Editing of new records can also be initiated with  {@link
 * com.smartgwt.client.widgets.grid.ListGrid#startEditingNew}, for example, from a button outside the grid. <P> <b>Saving
 * changes</b> <P> Saving of changes is triggered automatically when the user navigates out of the row or cell being edited
 * (based on {@link com.smartgwt.client.widgets.grid.ListGrid#getSaveByCell saveByCell}) or when the user ends editing.  
 * For a "mass update" interface, automatic saving of changes can be disabled entirely via  {@link
 * com.smartgwt.client.widgets.grid.ListGrid#getAutoSaveEdits 'autoSaveEdits:false'}, in which case a manual call to 
 * {@link com.smartgwt.client.widgets.grid.ListGrid#saveEdits} or {@link
 * com.smartgwt.client.widgets.grid.ListGrid#saveAllEdits} is required to trigger saving.  <P> If a grid has no DataSource,
 * saving means that the properties of the {@link com.smartgwt.client.widgets.grid.ListGridRecord}s in {@link
 * com.smartgwt.client.widgets.grid.ListGrid#getData 'grid.data'} are directly changed. <P> For a grid with a DataSource,
 * saving will be accomplished by using DataSource "update" operations for existing records, and DataSource "add"
 ...
 * an attempted save. <P> Similar to editing and saving, validation can be done on row transitions or on cell transitions
 * by setting {@link com.smartgwt.client.widgets.grid.ListGrid#getValidateByCell 'validateByCell'}, or can be disabled
 * entirely via {@link com.smartgwt.client.widgets.grid.ListGrid#getNeverValidate 'neverValidate:true'}. <P> <b>Editability
 * of cells</b> <P> Editors will either be shown for the complete row or for a single cell based on  {@link
 ...
 * via an override of {@link com.smartgwt.client.widgets.grid.ListGrid#canEditCell}. <P> Cells which are not editable just
 * display the cell's current value. <P> <b>Keyboard Navigation</b> <P> Full keyboard navigation is supported by default,
 * including Tab and Shift-Tab to navigate between cells in a row, and Up Arrow and Down Arrow to traverse rows.  Several
 * properties on both grids and fields, all named *EditAction, control navigation behavior of certain keys (eg Enter). <P>
 * You can use {@link com.smartgwt.client.widgets.grid.ListGrid#startEditing} to  programmatically move editing to a
 * particular cell, for example, during a  {@link com.smartgwt.client.widgets.grid.ListGridField#addChangedHandler} event.
 * <P> <b>editValues (unsaved changes)</b> <P> The term "editValues" means changes that the user has made to the dataset
 * which have not been saved.  The grid manages and stores editValues separately from the data itself in order to allow the
 * user to revert to original values, and in order to enable to grid to send only updated fields to the server.   <P>
 * Because editValues are stored separately, if you directly access the dataset (eg via  <code>grid.data.get()</code>) you
 * will see the records without the user's unsaved changes. Many APIs exist for retrieving and managing editValues (search
 * for editValue). For the common case of needing to access the record-as-edited, you can call  {@link
 ...
 * will be a {@link com.smartgwt.client.widgets.form.fields.FormItem}. The editor type for the cell will be determined by
 * {@link com.smartgwt.client.widgets.grid.ListGrid#getEditorType} based on the specified {@link
 * com.smartgwt.client.widgets.grid.ListGridField#getEditorType editorType} or {@link
 ...
 * com.smartgwt.client.widgets.grid.ListGridField#addChangeHandler}, {@link
 * com.smartgwt.client.widgets.grid.ListGridField#addChangedHandler} for detecting changes made by the user, {@link
 * com.smartgwt.client.widgets.grid.ListGrid#addCellChangedHandler} for detecting changes that have been successfully
 * saved, and {@link com.smartgwt.client.widgets.grid.ListGrid#addEditorEnterHandler} and {@link
 * com.smartgwt.client.widgets.grid.ListGrid#addEditorExitHandler} for detecting user navigation during editing. <P> You
 * can also install event handlers directly on the FormItem-based editors used in the grid via {@link
 * com.smartgwt.client.widgets.grid.ListGridField#getEditorProperties 'editorProperties'} as mentioned above.  When
 * handling events on items, or which involve items, be aware that in addition to standard  {@link
