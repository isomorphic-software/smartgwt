 ...
 * to build a complete application in the modern web architecture. <P> The Smart GWT Server can be {@link
 * com.smartgwt.client.docs.IscInstall 'integrated'} into any pre-existing Java application, and is designed to rapidly
 * connect Smart GWT visual components to pre-existing Java business logic or persistence engines.  Smart GWT's Ajax
 * request processing facilities can be easily integrated into Struts Actions, {@link
 * com.smartgwt.client.docs.SpringIntegration 'Spring controllers'}, or custom servlets and JSPs. <P> Alternatively, the
 * Smart GWT Server provides a complete SQL and Hibernate-based persistence engine for new applications, with
 * out-of-the-box servlets for processing Ajax data requests. <P> The Smart GWT Server is optional, and Smart GWT's
 * client-side Ajax engine can be integrated with any server that provides HTTP access, using XML, JSON, SOAP or
 * proprietary data protocols.  However any server in a modern web application will be required to provide most or all of
 * the features of the Smart GWT Server (described below), and the Smart GWT Server represents a best-of-breed
 * implementation of these facilities with a long history of high-volume production deployments. <P> <h4>Server enforcement
 * of {@link com.smartgwt.client.widgets.form.validator.Validator}</h4> <P> Data passed from the browser can be
 * automatically validated by the Smart GWT Server. In contrast, when using {@link
 ...
 * protocol</h4> <P> The Smart GWT Server provides a standardized request and response protocol designed for data-oriented
 ...
 * <P> Smart GWT provides a standard, {@link com.smartgwt.client.data.DataSourceField#getValueXPath 'XPath-based approach'}
