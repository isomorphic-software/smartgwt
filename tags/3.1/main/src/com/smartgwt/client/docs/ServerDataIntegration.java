 ...
 * after the Smart GWT processing</li>   <li>Use  Transaction Chaining to dynamically set data
 * values        according to the results of earlier transactions   <li>For SQL DataSources, use
 * {@link com.smartgwt.client.docs.CustomQuerying SQL Templating} to change, add        to or even
 * completely replace the SQL sent to the database, including calling       stored procedures</li>
 * <li>Use {@link com.smartgwt.client.docs.DmiOverview Direct Method Invocation} to call directly
 * into your own Java        classes.  DMIs allow you to modify the <code>DSRequest</code> before
 * it executes,       modify the <code>DSResponse</code> before it returns, or take unrelated
 * actions.  They       can be used to add business logic to a persistence operation without
 * destroying       the default behavior</li>    <li>Use {@link
