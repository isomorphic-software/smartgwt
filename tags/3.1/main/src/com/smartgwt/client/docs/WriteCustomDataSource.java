 ...
 * Out of the box, and with no code to write, Smart GWT supports SQL and Hibernate for 
 * persistence.  For other Java-based persistence systems, such as EJB, JPA, or systems
 * proprietary to your company, you write a custom DataSource class in Java.  In most cases, it is
 * possible to write a single, generic DataSource class that provides access to all data that is a
 * available from a given persistence mechanism; for example, a single DataSource class can
 * typically be written for accessing all Entity Beans available via EJB. <p> Note that a majority
 * of the features of the Smart GWT Server framework apply even when using your own persistence
