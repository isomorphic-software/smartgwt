 ...
 * com.smartgwt.client.data.DSRequest request metadata} (paging parameters, requested sort order, original values of data
 * being modified) and {@link com.smartgwt.client.data.DSResponse response metadata} (error handling, cache management,
 * session expiration etc). <P> Similar to the impact of Struts on early web applications, this standardized protocol
 * avoids developers in different groups inventing their own incompatible and redundant request/response protocols, and
 * allows developers to more easily learn code they didn't author. <P> <h4>Bi-directional XPath binding to Java
 * Objects</h4> <P> Most UI designs do not directly reflect the underlying Object model and so some degree of translation
 * is necessary in order to populate UI components with data and apply user changes to the Java Object model.  This is
 * often accomplished with brittle, difficult to understand data translation code sprinkled throughout the system, done in
 * a different way for every screen or component. <P> Smart GWT provides a standard, {@link
 * com.smartgwt.client.data.DataSourceField#getValueXPath XPath-based approach} to adapting any Java-based Object model to
 * the requirements of the UI design.  Data relevant to the application UI is centrally extracted in the server-side {@link
 * com.smartgwt.client.data.DataSource} layer, so that all UI components have a consistent, unified view of the data model
 * for both loading <b>and</b> saving data. <P> <h4>Broadest possible browser support</h4> <P> The Smart GWT Server can
 * compensate for facilities  {@link com.smartgwt.client.docs.PlatformDependencies missing or disabled in certain
 * browsers}, including ActiveX being disabled in IE6 and missing XML support in some versions of Apple's Safari browser.
 * <P> <h4>Transparent Proxying</h4> <P> {@link com.smartgwt.client.rpc.RPCManager#sendProxied Proxying} allows Smart GWT
 * applications to access web services, RSS feeds, HTML content and other data services in a secure manner regardless of
 * where they are located: across the enterprise or publicly available. <P> <h4>Optional {@link
