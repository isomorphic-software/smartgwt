 ...
 * behavior which has surprising effects when margins exist on both parents and children.  Compounding the problem, margins
 * are implemented very differently on different browsers, especially when it comes to HTML margins. <p> <b>Note about CSS
 * "box models"</b> <p> The CSS "box model" defines whether the size applied to a DOM element includes padding, borders or
 * margins, or whether such settings effectively <b>increase</b> the size of the component beyond the size specified in
 * CSS. <p> In Smart GWT, the size configured for a component <i>includes</i> border, padding and margins if specified (in
 * CSS terminology, the box model is "margin-box").  This allows CSS borders, margins and padding to be treated as purely
 * visual properties with no effect on sizing or layout.
 */
