 ...
 * com.smartgwt.client.widgets.grid.ListGrid#startEditing ListGrid.startEditing} or {@link
 * com.smartgwt.client.widgets.grid.ListGrid#startEditingNew ListGrid.startEditingNew}. <P> <b>New record creation</b> <P>
 * By default, editing is restricted to existing records.  Setting {@link
 ...
 * manual call to  {@link com.smartgwt.client.widgets.grid.ListGrid#saveEdits ListGrid.saveEdits} or {@link
 * com.smartgwt.client.widgets.grid.ListGrid#saveAllEdits ListGrid.saveAllEdits} is required to trigger saving.  <P> If a
 * grid has no DataSource, saving means that the properties of the {@link com.smartgwt.client.widgets.grid.ListGridRecord}s
 * in {@link com.smartgwt.client.widgets.grid.ListGrid#getData 'grid.data'} are directly changed. <P> For a grid with a
 ...
 * com.smartgwt.client.widgets.grid.ListGrid#saveAllEdits ListGrid.saveAllEdits} is called,  {@link
 * com.smartgwt.client.rpc.RPCManager#startQueue RPCManager.startQueue} will be automatically used to enable all edits to
 * be saved in one HTTP turnaround (if using the Smart GWT Server). <P> By default, a grid will send only updated fields
 * and the primaryKey field as part of  {@link com.smartgwt.client.data.DSRequest#getData data} so that the server can
 * discern which fields the user actually changed. However, the grid always includes the original field values in the
 * dsRequest as {@link com.smartgwt.client.data.DSRequest#getOldValues oldValues}. <P> Note that although it is possible to
 * load DataSource data without actually declaring a {@link com.smartgwt.client.data.DataSourceField#getPrimaryKey
 * 'primaryKey field'}, a primaryKey must be declared for editing and saving.  The primaryKey value is how Smart GWT
 * identifies the changed record to the server. <P> <b>Validation</b> <P> Any time saving is attempted, validation is
 * automatically triggered.  Values entered by the user will be checked against the {@link
 ...
 * com.smartgwt.client.widgets.grid.ListGrid}.  Whether a cell can be edited can be controlled on a per field basis by
 * setting {@link com.smartgwt.client.widgets.grid.ListGridField#getCanEdit 'field.canEdit'}, or on a per-record basis by
 * setting {@link com.smartgwt.client.widgets.grid.ListGrid#getRecordEditProperty 'recordEditProperty'} on a 
 * ${isc.DocUtils.linkForRef('object:ListGridRecord','record')}, or can be controlled on an arbitrary, programmatic basis
 * via an override of {@link com.smartgwt.client.widgets.grid.ListGrid#canEditCell ListGrid.canEditCell}. <P> Cells which
 * are not editable just display the cell's current value. <P> <b>Keyboard Navigation</b> <P> Full keyboard navigation is
 * supported by default, including Tab and Shift-Tab to navigate between cells in a row, and Up Arrow and Down Arrow to
 * traverse rows.  Several properties on both grids and fields, all named *EditAction, control navigation behavior of
 * certain keys (eg Enter). <P> You can use {@link com.smartgwt.client.widgets.grid.ListGrid#startEditing
 * ListGrid.startEditing} to  programmatically move editing to a particular cell, for example, during a  {@link
 * com.smartgwt.client.widgets.grid.ListGridField#addChangedHandler ListGridField.addChangedHandler} event. <P>
 * <b>editValues (unsaved changes)</b> <P> The term "editValues" means changes that the user has made to the dataset which
 * have not been saved.  The grid manages and stores editValues separately from the data itself in order to allow the user
 * to revert to original values, and in order to enable to grid to send only updated fields to the server.   <P> Because
 * editValues are stored separately, if you directly access the dataset (eg via  <code>grid.data.get()</code>) you will see
 * the records without the user's unsaved changes. Many APIs exist for retrieving and managing editValues (search for
 * editValue). For the common case of needing to access the record-as-edited, you can call  {@link
 * com.smartgwt.client.widgets.grid.ListGrid#getEditedRecord ListGrid.getEditedRecord}. <P> When accessing and manipulating
 * edited data, you should think carefully about whether you want to be working with the original data or with the edited
 * version.  Values entered by the user may not have been validated yet, or may have failed validation, hence you may find
 * a String value in a field of type "date" or "int", which could cause naive formatters or totaling functions to crash.
 * <P> Setting editValues is fully equivalent to the user making changes to data via the editing UI. If you <i>also</i>
 * allow editing external to the grid, setting editValues is one way to  combine changes from external editors into the
 * grid's edits, so that you can do a single  save. <P> <b>Customizing Cell Editors</b> <P> When a cell is being edited,
 * the editor displayed in the cell will be a {@link com.smartgwt.client.widgets.form.fields.FormItem}. The editor type for
 * the cell will be determined by {@link com.smartgwt.client.widgets.grid.ListGrid#getEditorType ListGrid.getEditorType}
 * based on the specified {@link com.smartgwt.client.widgets.grid.ListGridField#getEditorType editorType} or {@link
 ...
 * customize editing behavior.  Some of the most popular are {@link
 * com.smartgwt.client.widgets.grid.ListGridField#addChangeHandler ListGridField.addChangeHandler}, {@link
 * com.smartgwt.client.widgets.grid.ListGridField#addChangedHandler ListGridField.addChangedHandler} for detecting changes
 * made by the user, {@link com.smartgwt.client.widgets.grid.ListGrid#addCellChangedHandler ListGrid.addCellChangedHandler}
 * for detecting changes that have been successfully saved, and {@link
 * com.smartgwt.client.widgets.grid.ListGrid#addEditorEnterHandler ListGrid.addEditorEnterHandler} and {@link
 * com.smartgwt.client.widgets.grid.ListGrid#addEditorExitHandler ListGrid.addEditorExitHandler} for detecting user
 * navigation during editing. <P> You can also install event handlers directly on the FormItem-based editors used in the
 * grid via {@link com.smartgwt.client.widgets.grid.ListGridField#getEditorProperties 'editorProperties'} as mentioned
 * above.  When handling events on items, or which involve items, be aware that in addition to standard  {@link
