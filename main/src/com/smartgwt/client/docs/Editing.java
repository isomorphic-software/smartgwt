 ...
 * com.smartgwt.client.widgets.grid.ListGrid#getListEndEditAction listEndEditAction} to "next"
 * allows the user to create new records by simply navigating off the end of the dataset with the
 * keyboard.  Editing of new records can also be initiated with  {@link
 ...
 * com.smartgwt.client.widgets.grid.ListGrid#getSaveByCell saveByCell}) or when the user ends
 * editing.   For a "mass update" interface, automatic saving of changes can be disabled entirely
 * via  {@link com.smartgwt.client.widgets.grid.ListGrid#getAutoSaveEdits autoSaveEdits:false}, in
 * which case a manual call to  {@link com.smartgwt.client.widgets.grid.ListGrid#saveEdits
 * saveEdits()} or {@link com.smartgwt.client.widgets.grid.ListGrid#saveAllEdits saveAllEdits()}
 * is required to trigger saving.  <P> If a grid has no DataSource, saving means that the
 * properties of the {@link com.smartgwt.client.widgets.grid.ListGridRecord}s in {@link
 ...
 * of  {@link com.smartgwt.client.data.DSRequest#getData data} so that the server can discern
 * which fields the user actually changed. However, the grid always includes the original field
 * values in the dsRequest as {@link com.smartgwt.client.data.DSRequest#getOldValues oldValues}.
 * <P> Note that although it is possible to load DataSource data without actually declaring a
 * {@link com.smartgwt.client.data.DataSourceField#getPrimaryKey primaryKey field}, a primaryKey
 * must be declared for editing and saving.  The values of primaryKey fields is how Smart GWT
 * identifies the changed record to the server. <P> <b>Validation</b> <P> Any time saving is
 * attempted, validation is automatically triggered.  Values entered by the user will be checked
 * against the {@link com.smartgwt.client.widgets.grid.ListGridField#getValidators validators} and
 * the {@link com.smartgwt.client.data.DataSourceField#getValidators validators}. Any invalid
 * values abort an attempted save. <P> Similar to editing and saving, validation can be done on
 * row transitions or on cell transitions by setting {@link
 ...
 * com.smartgwt.client.widgets.grid.ListGrid#getRecordEditProperty recordEditProperty} on a 
 * {@link com.smartgwt.client.widgets.grid.ListGridRecord record}, or can be controlled on an
 * arbitrary, programmatic basis via an override of {@link
 ...
 * cell, for example, during a  {@link com.smartgwt.client.widgets.grid.ListGridField#changed
 * field.changed()} event. <P> <b>editValues (unsaved changes)</b> <P> The term "editValues" means
 * changes that the user has made to the dataset which have not been saved.  The grid manages and
 * stores editValues separately from the data itself in order to allow the user to revert to
 * original values, and in order to enable to grid to send only updated fields to the server.  
 * <P> Because editValues are stored separately, if you directly access the dataset (eg via 
 ...
 * com.smartgwt.client.widgets.grid.ListGridField#getType data type} for the field in question.
 * <P> You can customize the editor by setting {@link
 * com.smartgwt.client.widgets.grid.ListGridField#getEditorProperties editorProperties} to a set
 * of properties that is valid for that FormItem type.  Custom FormItem classes are also allowed,
 * for example, you may use {@link com.smartgwt.client.widgets.form.fields.FormItem#getIcons
 * icons} to create an icon that launches a separate {@link com.smartgwt.client.widgets.Dialog} in
 * order to provide an arbitrary interface that allows the user to select the value for a field.
 * <P> <b>Events</b> <P> Editing triggers several events which you can provide handlers for in
 * order to customize editing behavior.  Some of the most popular are {@link
 * com.smartgwt.client.widgets.grid.ListGridField#change field.change()}, {@link
 * com.smartgwt.client.widgets.grid.ListGridField#changed field.changed()} for detecting changes
 * made by the user, {@link com.smartgwt.client.widgets.grid.ListGrid#addCellChangedHandler
 * ListGrid.cellChanged} for detecting changes that have been successfully saved, and {@link
 ...
 * a per-row basis, use {@link com.smartgwt.client.widgets.grid.ListGrid#getSetEditorCustomizer
 * setEditorCustomizer}. <P> <b>NOTE:</b> with both APIs, in effect several FormItems are
 * generated from the customized FormItem you provide - see the docs for {@link
 * com.smartgwt.client.data.DataSourceField#getSetEditorType setEditorType} for special coding
 * patterns that apply in this case.  <P> <b>Binary Fields</b> <P> The ListGrid will automatically
 * show "view" and "download" icon buttons for binary field types (see {@link
