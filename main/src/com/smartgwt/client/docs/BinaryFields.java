 ...
 * DataSource.viewFile} can be used to programmatically trigger download/viewing as well. <P> As
 * covered under the {type:FieldType,"binary" field type description}, binary fields imply 3 other
 * related metadata fields named after the binary field, which store the file size, file name and
 * date of creation. <P> When using one of the built-in server DataSource types (SQL, JPA,
 * Hibernate), these metadata fields, UI controls and APIs will work with no special effort - just
 * declare a binary field.  For JPA or Hibernate the Java getter/setter for the binary field must
 * be of type byte[] or Byte[] (this is imposed by the ORM system); if you  are using the SQL
 * DataSource, no additional declarations are required unless you are using  beanClassName, in
 * which case you must added getters/setters of type InputStream, or one of the types listed for
