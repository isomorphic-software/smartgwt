 ...
 * <tr><td></td><td><i>IBM DB2 on i5 (aka iSeries, AS/400) - V5R4M0 onwards</i></td><td></td></tr> <tr><td></td><td><i>MS
 * SQL Server 2000, 2005, 2008</i></td><td></td></tr> <tr><td></td><td><i>MySQL 3.2.x, 4.0.x, 4.1.x, 5.0.x, 5.1.x,
 * 5.5.x</i></td><td></td></tr> <tr><td></td><td><i>Oracle 8.0.5, 8i, 9i, 10g, 11g</i></td><td></td></tr>
 * <tr><td></td><td><i>PostgreSQL 7.x, 8.x, 9.x</i></td><td></td></tr> </table> <p>You will also need a JDBC driver for
 * your specific database. Licensing restrictions prevent us including any JDBC driver other than the one for HSQLDB. 
 * However, you can download these drivers for free from the vendors' websites.  If your specific database server or
 * version is not listed above, please go to the <a href="http://forums.smartclient.com">Smart GWT forums</a> for
 * assistance. <P> You can also connect to an existing database table - the Visual Builder DataSource Wizard ("New" button
 * in DataSources pane) allows you to pick an existing database table and have a DataSource generated from it.  The Batch
 * DataSource Generator tool will allow you to generate DataSources for any number of SQL DataSources, and customize the
 * process so that organization-specific rules are applied (eg ignoring certain columns, or using naming conventions to
 * generate more complete DataSources). <P> Once you have your SQL DataSource connected to a table, in a default SDK
 * installation, DSRequests for your DataSource will be sent to the default {@link
 * com.smartgwt.client.rpc.RPCManager#actionURL actionURL}, and hence handled automatically, without you having to write
 * any Java code, by the {@link com.smartgwt.client.docs.ServletDetails IDACall servlet registered in web.xml}. IDACall is
 ...
 * see this description of {@link com.smartgwt.client.docs.ServerDataIntegration server data integration}.
