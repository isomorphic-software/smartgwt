 ...
 * <b>sql.pool.enabled</b> to false in {@link com.smartgwt.client.docs.Server_properties
 * server.properties}, or disabled for a specific database configuration via
 * <b>sql.<i>dbName</i>.pool.enabled</b>. The following properties can also be set on sql.pool /
 * sql.<i>dbName</i>.pool and control same-named DBCP properties, however, it is not recommended
 * to set these properties unless you have experience with DBCP and are troubleshooting a specific
 * pool-related performance problem: testOnBorrow, testOnReturn, testWhileIdle,
 * timeBetweenEvictionRunsMillis, minEvictableIdleTimeMillis, numTestsPerEvictionRun. <p> If you
 * are trying to diagnose an issue related to SQL connection pooling, you can enable DEBUG logging
 * for the following classes in <code>log4j.isc.config.xml</code> (see installation instructions
 * for details about this file).  All of these classes are in package
 * <code>com.isomorphic.sql</code>: <ul> <li> PoolableSQLConnectionFactory: logs connection
 * creation, and whether or not the      connections are pooled <li> SQLConnectionManager: logs
 * when connections are borrowed <li> SQLDriver: logs the hashCode of the connection when SQL
 * statements are executed <li> SQLTransaction: logs transactional open, commit, rollback and
 * close. </ul> <li>JPA/Hibernate: Hibernate's built-in connection pool is <b>not</b> intended for
 ...
 * production use you must use production-ready connection pool libraries for example C3P0. Here
 * are recommended settings for C3P0 properties: <ul> <li>c3p0.acquireRetryDelay=1000</li>
