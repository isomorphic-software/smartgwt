 ...
 * Transfer Objects</a>) and  other unnecessary scaffolding.  Ibatis avoids some of this
 * scaffolding, but requires every  SQL query to be written by hand.  In contrast the SQL
 * DataSource supports basic CRUD queries out of the box. <p> <b>Performance</b> <p> Systems like
 * JPA work nicely when dealing with a single object at a time, but enterprise applications
 * routinely work with lists or trees of objects that draw data from multiple tables.  In these
 * situations, it's trivial to express an efficient SQL query for retrieving the desired results
 * (as shown in @see <a href="http://www.smartclient.com/smartgwtee/showcase/#largeValueMap"
 * target="examples">this example</a>).  Fetching the same data using getter methods on Java Beans
 * often leads to nightmare performance scenarios (such as 3 or more separate SQL queries per
 * object retrieved). <P> Trying to "trick" the persistence system into generating efficient
 * queries doesn't make sense - this just leads to a far more complex and fragile solution that
 * now requires deep knowledge of how the ORM system generates SQL as well as SQL itself.  <P>
 * SQLDataSource allows you to directly write SQL when it makes sense, and  {@link
 ...
 * Transaction Chaining, you can chain together multiple SQL  queries, or a mixture of SQL queries
 * and other data access, with simple declarations right  in the DataSource, as <a
