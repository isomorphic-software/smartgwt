...
/**
 * <h3>SQL DataSource vs JPA, EJB, MyBatis and other technologies</h3>
 ...
 * the Smart GWT SQL DataSource, simple CRUD connectivity can easily be set up in a way that
 * requires zero server side code.  Only a DataSource descriptor (.ds.xml file) needs to exist;
 * this descriptor can include field-level details explicitly, if you choose to provide them, or
 * can be inferred from your schema via {@link
 * com.smartgwt.client.docs.serverds.DataSource#autoDeriveSchema DataSource.autoDeriveSchema}. 
 * The descriptor actually serves double duty by also providing the configuration for UI
 ...
 * Objects</a>) and  other unnecessary scaffolding.  MyBatis avoids some of this scaffolding, but
