 ...
 * clipping or scrolling regions, so that a scrolling grid shows all rows in the cached range
 * around the first visible row <li> removes certain decorative images, such as image-based
 * backgrounds, which may print poorly in black and white <li> converts editing controls into
 * static representations of the data being edited <li> removes interactive elements such as
 * buttons and menus, which don't work on paper and waste space </ul> The default printed view can
 * be customized with settings and method overrides as necessary, including the ability to created
 * printed representations of custom components you have created. <P> For simple, built in
 * printing support, see the {@link com.smartgwt.client.widgets.Canvas#showPrintPreview
 * Canvas.showPrintPreview()} and {@link com.smartgwt.client.widgets.Canvas#getPrintPreview
 * Canvas.getPrintPreview()} APIs, or for finer grained control developers may call {@link
 * com.smartgwt.client.widgets.Canvas#getPrintHTML Canvas.getPrintHTML()} directly and work with
 * the {@link com.smartgwt.client.widgets.PrintCanvas} or {@link
 * com.smartgwt.client.widgets.PrintWindow} class. <P> Note that the {@link
 * com.smartgwt.client.widgets.cube.CubeGrid} component does not currently support WYSIWYG 
 * printing (as documented {@link com.smartgwt.client.widgets.cube.CubeGrid#getPrintHTML in that
 * class}).
 ...
 * @see com.smartgwt.client.widgets.Canvas#print
